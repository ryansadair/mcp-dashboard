...
def render_dividends_tab(tamarac_parsed, active_strategy, strat_config, kpis):
    ...
    # ═══════════════════════════════════════════════════════════════════════
    with sub_announce:
        # Show summary KPIs across the top (existing pattern)
        ...
        with d2: st.metric("Avg 1Y Div Growth", f"{_avg_growth('div_growth_1y'):+.1f}%")
        with d3: st.metric("Avg 3Y Div Growth", f"{_avg_growth('div_growth_3y'):+.1f}%")
        with d4: st.metric("Avg 5Y Div Growth", f"{_avg_growth('div_growth_5y'):+.1f}%")
...
def _render_dividend_detail(edf, active_strategy, strat_color):
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Div Rate":      st.column_config.NumberColumn("Rate", format="$%.2f"),
